<template>
  <div>
    ...
    <div class="calendar-weekTime">
      <span class="calendar-weekTime-cancel" v-on:click="reseting">撤销所有操作</span>
...
export default {
  ...
  created(){
    ...
    //   this.data = this.initDataFormat();
    // }
    // console.log(this.initDataFormat())
  },
  watch:{
    data(val){
      this.timeBucket=val
  ...
  data(){
    return{
      ...
      timeBucket:this.data,
    }
  },
  methods:{
    initDataFormat(){
      let weeks = ['一','二','三','四','五','六','日'],initData = [];
      let times = 48;
